...
<#macro image individual propertyGroups namespaces editable showPlaceholder="never">
    ...
    <#-- Check local property for image first; then check mainImage -->
    <#assign photo = propertyGroups.pullProperty("http://vivo.brown.edu/ontology/vivo-brown/photo")!>
    <#if photo?has_content >
	   <img class="individual-photo" src="${photo.statements[0].value}" alt="${individual.name}" width="160" />
    ...
    <#elseif (mainImage.statements)?has_content && thumbUrl?has_content>
